...
class IPythonInterpreter(BaseTool):
    ...
    code: str = Field(..., description="Python code to execute (multi-line supported, separated with newline character).")

    async def run(self) -> str:
        # :brain: Retrieve or create persistent shell from shared context
...
if __name__ == "__main__":
    ...
    tool = IPythonInterpreter(code="x = 42\nx = x + 1\nx")
    tool._context = ctx  # manually attach context for testing
    tool2 = IPythonInterpreter(code="x + 1")
    tool2._context = ctx

    print(asyncio.run(tool.run()))
    print(asyncio.run(tool2.run()))  # persists `x` across runs